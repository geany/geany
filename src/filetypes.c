...
static GeanyFiletypeGroupID get_filetype_group(const gchar *title, const gchar *name)
{
	...
	ret = get_group(title);
	if (ret != GEANY_FILETYPE_GROUP_NONE)
		return ret;

	/* Currently only because of "(O)Caml" where title starts with '(' and not
	 * a letter */
...
static void init_builtin_filetypes(void)
{
	...
	FT_INIT( CAML,         OCAML,        "CAML",             "(O)Caml"                 );
	...
	FT_INIT( SH,           SH,           "Sh",               _("Shell")                );
	FT_INIT( MAKE,         MAKEFILE,     "Make",             _("Makefile")             );
	FT_INIT( XML,          NONE,         "XML",              NULL                      );
	FT_INIT( DOCBOOK,      DOCBOOK,      "Docbook",          NULL                      );
	FT_INIT( HTML,         HTML,         "HTML",             NULL                      );
	FT_INIT( CSS,          CSS,          "CSS",              _("Cascading Stylesheet") );
	...
	FT_INIT( CONF,         CONF,         "Conf",             _("Config")               );
	FT_INIT( PO,           NONE,         "Po",               _("Gettext translation")  );
