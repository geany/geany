...
static void save_dialog_prefs(GKeyFile *config)
{
	...
	/* printing */
	g_key_file_set_string(config, "printing", "print_cmd2", printing_prefs.external_print_cmd ? printing_prefs.external_print_cmd : "");
...
static void load_dialog_prefs(GKeyFile *config)
{
	...
	/* new value is print_cmd2 with build-like format, old is print_cmd with only %f for full path */
	if (g_key_file_has_key(config, "printing", "print_cmd2", NULL))
		printing_prefs.external_print_cmd = utils_get_setting_string(config, "printing", "print_cmd2", tmp_string);
	else /* load old value */
	{
