...
static gboolean has_tabs_on_left(GeanyDocument *doc)
{
...
static void on_close_documents_left_activate(GtkMenuItem *menuitem, GeanyDocument *doc)
{
	g_return_if_fail(has_tabs_on_left(doc));
	GtkNotebook *nb = GTK_NOTEBOOK(main_widgets.notebook);
	gint current_page = gtk_notebook_get_current_page(nb);
	gint doc_page = document_get_notebook_page(doc);
	for (gint i = doc_page - 1; i >= 0; i--)
	{
	...
	gtk_notebook_set_current_page(nb, MAX(current_page, 0));
}
...
static void show_tab_bar_popup_menu(GdkEventButton *event, GeanyDocument *doc)
{
	...
	menu_item = ui_image_menu_item_new(GTK_STOCK_CLOSE, _("Close Documents to the _Left"));
	gtk_widget_show(menu_item);
	gtk_container_add(GTK_CONTAINER(menu), menu_item);
	g_signal_connect(menu_item, "activate", G_CALLBACK(on_close_documents_left_activate), doc);
	gtk_widget_set_sensitive(GTK_WIDGET(menu_item), doc != NULL && has_tabs_on_left(doc));
