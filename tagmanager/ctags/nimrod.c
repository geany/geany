...
*
*   This module contains functions for generating tags for Nimrod language
*   files.
*/
/*
*   INCLUDE FILES
	...
	vString *const parent, int is_class_parent, const char *arglist)
{
	...
	tag.kindName = NimrodKinds[K_FUNCTION].name;
	tag.kind = NimrodKinds[K_FUNCTION].letter;
	...
	if (vStringLength (parent) > 0)
	{
		if (is_class_parent)
		{
			tag.kindName = NimrodKinds[K_METHOD].name;
			tag.kind = NimrodKinds[K_METHOD].letter;
	...
	vString *const parent, int is_class_parent)
{
	...
	tag.kindName = NimrodKinds[K_CLASS].name;
	tag.kind = NimrodKinds[K_CLASS].letter;
	if (vStringLength (parent) > 0)
	{
		if (is_class_parent)
		{
...
static void makeVariableTag (vString *const var, vString *const parent)
{
	...
	tag.kindName = NimrodKinds[K_VARIABLE].name;
	tag.kind = NimrodKinds[K_VARIABLE].letter;
	if (vStringLength (parent) > 0)
	{
		tag.extensionFields.scope [0] = "class";
		tag.extensionFields.scope [1] = vStringValue (parent);
...
static void parseImports (const char *cp)
{
	...
	while (*cp)
	{
		...
		{
			makeSimpleTag (name, NimrodKinds, K_IMPORT);
		}
	}
	vStringDelete (name);
...
static void findNimrodTags (void)
{
