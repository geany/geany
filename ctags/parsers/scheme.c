	...
	while ((line = readLineFromInputFile ()) != NULL)
	{
			...
			(isspace (cp [5]) || cp[5] == '\0'))
		{
			...
			if (line == NULL)
				break;
			readIdentifier (name, cp);
			makeSimpleTag (name, SchemeKinds, K_VARIABLE);
		}
	}
	vStringDelete (name);
}
...
{
	static const char *const extensions [] = {"SCM", "SM", "sch", "scheme", "scm", "sm", NULL};
	//static const char *const aliases [] = {"gosh", "guile", NULL};
