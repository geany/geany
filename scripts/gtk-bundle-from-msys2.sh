...
_getpkg() {
	if [ "$use_cache" = "yes" ]; then
		...
		"cairo")
			...
			echo "https://mirror.msys2.org/mingw/$(_mingw_base_dir)/mingw-w64-${ABI}-cairo-1.18.4-1-any.pkg.tar.zst"
			;;
		"pango")
			# stick with pango-1.56.3-2 until lagging issue resolved
			# https://github.com/geany/geany/pull/4360
			echo "https://mirror.msys2.org/mingw/$(_mingw_base_dir)/mingw-w64-${ABI}-pango-1.56.3-2-any.pkg.tar.zst"
			;;
		*)
